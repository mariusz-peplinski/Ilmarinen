...
const applyAttackToActors = (
  ...
  for (const actor of actorStates.values()) {
    if (!isActorInsideActionBox(actor, event, ATTACK_ACTION_PROFILE)) {
      continue;
    }

    const shape = getActorCollisionShape(actor);
    const knockbackX = shape.knockbackable
      ? event.directionX * ATTACK_ACTION_PROFILE.knockback * shape.knockbackScale
      : 0;
    const knockbackY = shape.knockbackable
      ? event.directionY * ATTACK_ACTION_PROFILE.knockback * shape.knockbackScale
      : 0;
...
const applyInteractToActors = (
  ...
  for (const actor of actorStates.values()) {
    if (!isActorInsideActionBox(actor, event, INTERACTION_ACTION_PROFILE)) {
      continue;
    }
